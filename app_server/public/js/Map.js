...
//        return game.make.renderTexture(3548, 2048, 'terrainBackground');
    ...
    spawnRenderTextureTiles() {//DEPRECATED
        game.terrainRenderTexture = game.add.renderTexture(3548, 2048, 'terrainBackground');
        var noteTile = game.make.sprite(0, 0, 'notebookPaper');
        noteTile.anchor.set(.5);
        for (var nx = 0; nx < 4096; nx += 1132) {
            for (var ny = 0; ny < 4096; ny += 654) {
                game.terrainRenderTexture.renderXY(noteTile, nx, ny);
                game.terrainRenderTexture.renderXY(noteTile, nx + 1132 / 2, ny + 654 / 2);
            }
        }
        for (var xx = 0; xx < 2048; xx += 45) {
            for (var yy = 0; yy < 2048; yy += 45) {
                ...
                //console.log(point);
                game.terrainRenderTexture.renderXY(tile, point.x, point.y);
            }
        }
        game.add.sprite(0, 0, game.terrainRenderTexture);
    }
}
